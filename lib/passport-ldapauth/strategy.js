...
var handleAuthentication = function(req, options) {
  var username, password, ldap, serverClone,i;
  options || (options = {});
  ...
  if (this.options.useDirectBind) {
	  serverClone={};
	  for (i in this.options.server) {
		if (this.options.server.hasOwnProperty(i)) {
			serverClone[i]=this.options.server[i];
		}
	  }
	  
	  serverClone.bindDn=this.options.server.bindDn.replace(/{{username}}/g, sanitizeInput(username));
	  serverClone.bindCredentials=password;
	  
	  console.log("Server Clone: " + JSON.stringify(serverClone));
  } else {
	serverClone=this.options.server;
  }
  
  ldap = new LdapAuth(serverClone);
